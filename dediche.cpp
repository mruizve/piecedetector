...
static void CannyThreshold(int, void*)
{
  char str[300];
    ...
    dilation_element =  cv::getStructuringElement(2,
                        cv::Size(2*dilation_size+1,2*dilation_size+1),
                        cv::Point(dilation_size,dilation_size));

    std::vector< std::vector<cv::Point> > contours;
    std::vector<cv::Vec4i> hierarchy;
   std::vector< std::vector<cv::Point>  >standardShapes;
...
//    std::cout << minv << " " << maxv << "\n";
    ...
    cv::dilate( dst,dst,dilation_element);
    cv::erode( dst,dst,dilation_element);

      cv::findContours( dst, contours, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, cv::Point(0, 0) );
      cv::Mat img_shapes( dst.rows, dst.cols, CV_8UC3, cv::Scalar( 255, 255, 255 ) );
      for( size_t i = 0; i< contours.size(); i++ ){
        cv::Point centre = contourCentre(contours[i]);
        cv::Scalar color = Scalar( cv::rng.uniform(0, 255), cv::rng.uniform(0,255), cv::rng.uniform(0,255) );
        cv::drawContours( img_shapes, contours, i, color, 2, 8, hierarchy, 0, Point() );
        float val;
        int shapeId = whatShapeIs(standardShapes,contours[i],&val);
        sprintf(str,"shape %zu id=%i %.3lf ",i,shapeId,val);
        cv::putText(img_shapes,str,centre,1,1,color);
      }
	    cv::resize(img_shapes, img_shapes, Size(dst.cols*0.6, dst.rows*0.6));
	    imshow( window_name, img_shapes);
}
...
int main(int argc, char* argv[])
{
    ...
    imread("source2.jpg",IMREAD_GRAYSCALE).copyTo(img);
